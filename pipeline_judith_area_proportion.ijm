...
run("Set Measurements...", "area mean standard modal min center perimeter bounding integrated area_fraction display redirect=None decimal=3");
for(i=0; i<list.length;i++){
	file = list[i];
    open(dir + file);
    //wait(1000);
...
saveAs("Results", dir2+"result_area_proportion.csv");
